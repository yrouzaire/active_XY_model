...
## Investigating Statistics of Δθ and their link with Δω at a given time
L = 200
    T = 0.2
    Var = 0.2
    init = "LowTemp"
    lattice,dt = create_Lattice(L,T,Var,init) ; thetas = lattice[:,:,1] ; omegas = lattice[:,:,2]
    tmax = 1000 ; t = 0.0
    z = @elapsed while t<tmax
        global thetas = update(thetas,omegas,L,T,dt)
        global t += dt
    end
    println("Runtime = $z seconds")

Δθ = Float64[]
Δw = Float64[]
for i in 1:L
    for j in 1+i%2 : 2 : L
        a,b,c,d = -arclength.(mod.(get_angle_nearest_neighbours(thetas,i,j,L),2π),thetas[i,j])
        push!(Δθ,a,b,c,d)
        a,b,c,d = get_omega_nearest_neighbours(omegas,i,j,L) .- omegas[i,j]
        push!(Δw,a,b,c,d)
    end
end
Δθmod = mod.(Δθ .+ π,2π) .- π
using StatsBase,StatsPlots,SpecialFunctions

plot(xlabel="Δω",ylabel="Δθ")
    scatter!(Δw,Δθmod,ms=1)
    covm = cov(hcat(Δw,Δθmod))
    E = eigen(covm)
    tt = 0:0.01:2π
    xx = sqrt(E.values[1])*cos.(tt)
    yy = sqrt(E.values[2])*sin.(tt)
    α = atan(E.vectors[2,1],E.vectors[1,1])
    R = [cos(α) sin(α); -sin(α) cos(α) ]
    conf = 0.685 ; s = -2log(1-conf) # Mahalanobis radius of the ellipsoid
    plot!(sqrt(s)*hcat(xx,yy)*R[:,1],sqrt(s)*hcat(xx,yy)*R[:,2],c=:black,line=:solid)
    conf = 0.95 ; s = -2log(1-conf)
    plot!(sqrt(s)*hcat(xx,yy)*R[:,1],sqrt(s)*hcat(xx,yy)*R[:,2],c=:black,line=:dash)
    conf = 0.997 ; s = -2log(1-conf)
    plot!(sqrt(s)*hcat(xx,yy)*R[:,1],sqrt(s)*hcat(xx,yy)*R[:,2],c=:black,line=:dot)
    annotate!(1.7,-2.9,text("r=$(round(covm[1,2]/sqrt(covm[1,1]*covm[2,2]),digits=2))"))
# Question : can the variance be explained by the temperature fluctuations ?

## Investigating Statistics of Δθ averaged over time (short times) and their link with Δω
L = 200
    T = 0.
    Var = 0.3
    init = "HighTemp"
    lattice,dt = create_Lattice(L,T,Var,init) ; thetas = lattice[:,:,1] ; omegas = lattice[:,:,2]
    nt = 1 ; thetas_time = Array{Float64,3}(undef,L,L,nt)
    tmax = 100 ; t = 0.0
    z = @elapsed while t<tmax
        global thetas = update(thetas,omegas,L,T,dt)
        global t += dt
    ...
    for n in 1:nt
        global thetas = update(thetas,omegas,L,T,dt)
        global thetas_time[:,:,n] = thetas
    end
    println("Runtime = $z seconds")


thetas_avg = thetas
# thetas_avg = mod.(mean(thetas_time,dims=3)[:,:,1] .+ π,2π) .- π
# thetas_std = std(thetas_time,dims=3)[:,:,1] ./ thetas_avg * 100
# sort(vec(thetas_std),rev=true)
Δθ = Float64[] ; Δw = Float64[]
for i in 1:L
    for j in 1+i%2 : 2 : L
        a,b,c,d = -arclength.(mod.(get_angle_nearest_neighbours(thetas_avg,i,j,L),2π),thetas_avg[i,j])
        push!(Δθ,a,b,c,d)
        a,b,c,d = get_omega_nearest_neighbours(omegas,i,j,L) .- omegas[i,j]
        push!(Δw,a,b,c,d)
    end
...
p1 = plot(xlabel="Δω",ylabel="Δθ",xlims=(-2.1,2.1),ylims=(-1.8,1.8))
    Δθmod = mod.(Δθ .+ π,2π) .- π
    scatter!(Δw,Δθmod,ms=1)
    covm = cov(hcat(Δw,Δθmod))
    E = eigen(covm)
    ...
    conf = 0.997 ; s = -2log(1-conf)
    plot!(sqrt(s)*hcat(xx,yy)*R[:,1],sqrt(s)*hcat(xx,yy)*R[:,2],c=:black,line=:dot)
    r = round(covm[1,2]/sqrt(covm[1,1]*covm[2,2]),digits=2)
    annotate!(1.25,-1.5,text("r=$r"))
p2 = plot(xlabel="Δω",ylabel="⟨Δθ⟩",xlims=(-2.1,2.1),ylims=(-1.8,1.8))
    Δθmod = mod.(Δθ .+ π,2π) .- π
    scatter!(Δw,Δθmod,ms=1)
    covm = cov(hcat(Δw,Δθmod))
    E = eigen(covm)
